...
function searchUrl(){   //点击搜索按钮，判断两个下拉框的状态进行选择调用不同的函数(向服务器请求数据)
    ...
    //通过判断两个下拉框中的值，调用不同方法
    questionShow_subject=$("#subjectSelect").val();
    questionShow_point=$("#pointSelect").val();
    ...
    $.ajax({//数据线不填充
        ...
        type: "GET",
        data: {
            mohu:questionShow_subject,
            details:questionShow_point,
        },
...
$(function () {
    ...
    function user_show_showDetails(obj) {//修改用户
        var name=$(obj).parent().prev().prev().prev().prev().text();
         window.location.href='/pan/updateuser?username='+name;
        console.log(name);

    }
    function user_show_file(obj) {
        var name=$(obj).parent().prev().prev().prev().prev().text();
        window.location.href='/pan/userfile?username='+name;
        console.log(name);
    ...
    function user_show_Delete(obj) { //"删除按钮"删除错题图片
